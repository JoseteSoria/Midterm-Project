...
class StudentCheckingAccControllerImplTestUnit {
    ...
    void setUp() {
        ...
        doAnswer(i -> {
            return null;
        }).when(studentCheckingAccService).addBalance(ah1, ac1.getId(), new BigDecimal("100"), null, "ksdhuf", "skdhfs");
        doAnswer(i -> {
            return null;
        }).when(studentCheckingAccService).reduceBalance(ah1, ac1.getId(), new BigDecimal("100"), null, "ksdhuf", "skdhfs");
